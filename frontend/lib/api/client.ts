...
export async function apiCall<T = unknown>(
  ...
  const shouldQueueRequest = await shouldQueueRequest(options);

  if (shouldQueueRequest && typeof navigator !== 'undefined' && navigator.onLine === false) {
    const action = await queueOfflineAction(endpoint, options);
    registerBackgroundSync();
  ...
  for (let attempt = 0; attempt <= config.maxRetries; attempt++) {
    try {
      ...
      if (shouldQueueRequest && isLikelyOfflineError(lastError)) {
        const action = await queueOfflineAction(endpoint, options);
        registerBackgroundSync();
        throw new OfflineActionQueuedError(
